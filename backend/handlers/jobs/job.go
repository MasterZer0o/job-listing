...
type JobData struct {
	...
	ContentSections []string    `json:"contentSections"`
...
func GetJob(ctx *fiber.Ctx) error {
	...
	row := db.DB.QueryRow(ctx.Context(), `SELECT title, salary_from, salary_to,level, remote_available, companies.name, companies.image, companies.id, jobs.created_at, array_agg(tech_skills.name), (SELECT EXISTS (SELECT saved_jobs.job_id FROM sessions
		...
		LIMIT 1)), COALESCE(jobs_content.content, ''), COALESCE(jobs_content.sections, '{}') FROM jobs
  ...
  JOIN tech_skills ON tech_skills.id = job_tech_skills.tech_skill_id
	LEFT JOIN jobs_content ON jobs_content.job_id=$2 
	WHERE jobs.id = $2
  GROUP BY jobs.title, jobs.salary_from, jobs.salary_to, jobs.level, jobs.remote_available,companies.name,companies.image,companies.id, jobs.created_at, jobs_content.content, jobs_content.sections`, ctx.Cookies("session"), jobId)

	slog.Info("Time taken to query job", "job id", jobId, "time", time.Since(t))

	var data JobData
	err := row.Scan(&data.Title, &data.SalaryFrom, &data.SalaryTo, &data.Level, &data.RemoteAvailable, &data.Company.Name, &data.Company.Image, &data.Company.Id, &data.CreatedAt, &data.Skills, &data.IsSaved, &data.Content, &data.ContentSections)
